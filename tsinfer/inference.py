...
class Matcher(object):
    ...
    def get_ancestors_tree_sequence(self):
        """
        Return the ancestors tree sequence. In this tree sequence only inference sites
        are present, so site indexes all refer to inference sites only. All ancestral and
        derived states are 0/1. All nodes have the sample flag bit set.
        """
...
class SampleMatcher(Matcher):
    ...
    def __process_sample(self, sample_id, haplotype, thread_index=0):
        ...
        first, last = trim_region(haplotype, trim_value=tskit.MISSING_DATA)
        if first < last:
            # There is some usable data
            self._find_path(sample_id, haplotype, first, last, thread_index)
            match = self.match[thread_index]
            # Add the mutations in to the tree
            diffs = np.where(
                np.logical_and(haplotype != match, haplotype != tskit.MISSING_DATA))[0]
